import React from 'react';
import { MigrationElement, MigrationTableCell } from '@/types';
import { api } from '@/lib/api';
import { Table, Image as ImageIcon, HelpCircle, AlertCircle } from 'lucide-react';
import { clsx } from 'clsx';

interface ElementRendererProps {
  element: MigrationElement;
  documentId: string;
  index?: number;
}

export const ElementRenderer: React.FC<ElementRendererProps> = ({
  element,
  documentId,
  index = 0,
}) => {
  // Attached icons renderer
  const renderAttachedIcons = () => {
    if (!element.icons || element.icons.length === 0) return null;
    return (
      <div className="flex items-center gap-1.5 mb-1.5">
        {element.icons.map((iconPath, idx) => {
          const iconFileName = iconPath.split('/').pop() || iconPath;
          const url = api.getAssetUrl(documentId, iconFileName);
          return (
            <img
              key={idx}
              src={url}
              alt="Extracted Icon"
              className="size-5 object-contain rounded bg-white/5 p-0.5 border border-border"
              onError={(e) => {
                // If asset fails to load, hide gracefully
                (e.target as HTMLElement).style.display = 'none';
              }}
            />
          );
        })}
      </div>
    );
  };

  switch (element.element_type) {
    case 'heading': {
      const level = Math.min(Math.max(element.level || 2, 1), 6) as 1 | 2 | 3 | 4 | 5 | 6;
      const HeadingTag = `h${level}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';

      const headingClasses = {
        1: 'text-2xl font-bold tracking-tight text-text-main mt-6 mb-3',
        2: 'text-xl font-semibold text-text-main mt-5 mb-2.5',
        3: 'text-lg font-semibold text-primary mt-4 mb-2',
        4: 'text-base font-medium text-text-main mt-3 mb-1.5',
        5: 'text-sm font-semibold text-text-muted mt-2 mb-1',
        6: 'text-xs font-bold uppercase tracking-wider text-text-muted mt-2 mb-1',
      }[level] || 'text-lg font-semibold text-text-main';

      return (
        <div className="py-1">
          {renderAttachedIcons()}
          <HeadingTag className={headingClasses}>
            {element.text}
          </HeadingTag>
        </div>
      );
    }

    case 'paragraph': {
      return (
        <div className="py-1.5 leading-relaxed text-sm text-text-main/90">
          {renderAttachedIcons()}
          <p className="whitespace-pre-line">{element.text}</p>
        </div>
      );
    }

    case 'list': {
      const items = element.items || (element.text ? [element.text] : []);
      return (
        <div className="py-2">
          {renderAttachedIcons()}
          <ul className="space-y-1.5 pl-5 list-disc text-sm text-text-main/90 marker:text-primary">
            {items.map((item, idx) => (
              <li key={idx} className="leading-relaxed">
                {item}
              </li>
            ))}
          </ul>
        </div>
      );
    }

    case 'table': {
      const cells = element.cells || [];
      const numRows = element.num_rows || 1;
      const numCols = element.num_cols || 1;

      // Group cells by row_index
      const rows: Record<number, MigrationTableCell[]> = {};
      cells.forEach((cell) => {
        if (!rows[cell.row_index]) rows[cell.row_index] = [];
        rows[cell.row_index].push(cell);
      });

      return (
        <div className="my-4 space-y-2">
          {renderAttachedIcons()}
          {element.title && (
            <div className="flex items-center gap-2 text-xs font-semibold text-text-main">
              <Table className="size-3.5 text-primary" />
              <span>{element.title}</span>
            </div>
          )}

          <div className="overflow-x-auto rounded-lg border border-border bg-black/20">
            <table className="w-full border-collapse text-left text-xs">
              <tbody>
                {Object.keys(rows)
                  .map(Number)
                  .sort((a, b) => a - b)
                  .map((rIdx) => {
                    const rowCells = rows[rIdx].sort((a, b) => a.col_index - b.col_index);
                    const isHeaderRow = rIdx === 0 && rowCells.some((c) => c.is_header);

                    return (
                      <tr
                        key={rIdx}
                        className={clsx(
                          'border-b border-border/50 transition hover:bg-white/[0.02]',
                          isHeaderRow && 'bg-primary/10 font-semibold text-primary'
                        )}
                      >
                        {rowCells.map((c, cIdx) => {
                          const CellTag = c.is_header ? 'th' : 'td';
                          return (
                            <CellTag
                              key={cIdx}
                              rowSpan={c.row_span || 1}
                              colSpan={c.col_span || 1}
                              className={clsx(
                                'border-r border-border/40 px-3 py-2.5 align-top leading-relaxed',
                                c.is_header
                                  ? 'bg-primary/[0.08] font-semibold text-primary'
                                  : 'text-text-main'
                              )}
                            >
                              {c.text}
                            </CellTag>
                          );
                        })}
                      </tr>
                    );
                  })}
              </tbody>
            </table>
          </div>
        </div>
      );
    }

    case 'image': {
      const fileName =
        element.asset_filename ||
        (element.image_path ? element.image_path.split('/').pop() : '') ||
        '';
      const url = fileName ? api.getAssetUrl(documentId, fileName) : '';

      return (
        <div className="my-4 rounded-xl border border-border bg-black/20 p-4 space-y-2">
          {renderAttachedIcons()}
          <div className="flex items-center justify-center min-h-[140px] rounded-lg bg-card/60 overflow-hidden">
            {url ? (
              <img
                src={url}
                alt={element.title || 'Extracted document graphic'}
                className="max-h-96 max-w-full object-contain rounded"
                onError={(e) => {
                  (e.target as HTMLElement).style.display = 'none';
                }}
              />
            ) : (
              <div className="flex flex-col items-center gap-2 text-text-muted p-6">
                <ImageIcon className="size-8 opacity-50" />
                <span className="text-xs">Image graphic placeholder</span>
              </div>
            )}
          </div>
          {element.title && (
            <div className="text-center text-xs text-text-muted italic">
              {element.title}
            </div>
          )}
        </div>
      );
    }

    case 'icon': {
      const fileName =
        element.asset_filename ||
        (element.image_path ? element.image_path.split('/').pop() : '') ||
        '';
      const url = fileName ? api.getAssetUrl(documentId, fileName) : '';

      return (
        <div className="inline-flex items-center gap-2 rounded border border-border bg-black/10 px-2 py-1 my-1">
          {url && (
            <img
              src={url}
              alt="Icon"
              className="size-4 object-contain"
              onError={(e) => {
                (e.target as HTMLElement).style.display = 'none';
              }}
            />
          )}
          {element.text && <span className="text-xs text-text-main">{element.text}</span>}
        </div>
      );
    }

    default:
      return (
        <div className="my-2 rounded-lg border border-amber-500/30 bg-amber-500/[0.05] p-3 text-xs text-amber-300 flex items-start gap-2">
          <HelpCircle className="size-4 shrink-0 mt-0.5" />
          <div>
            <span className="font-semibold uppercase">{(element as any).element_type}</span>: {element.text || 'Structured item'}
          </div>
        </div>
      );
  }
};
